...
// Add /wifi/status endpoint for connected ip address query
//
...
// WiFi 狀態 handler - 查詢 STA 連線狀態與 IP
static esp_err_t wifi_status_handler(httpd_req_t *req) {
  ...
  *p++ = '{';
  ...
  p += sprintf(p, "\"sta_connected\":%s,", sta_connected ? "true" : "false");
  ...
  if (sta_connected) {
    ...
    p += sprintf(p, "\"sta_ip\":\"\",");
    p += sprintf(p, "\"sta_ssid\":\"\",");
  }
  
  p += sprintf(p, "\"ap_ip\":\"%s\",", WiFi.softAPIP().toString().c_str());
  p += sprintf(p, "\"ap_ssid\":\"ESP32-CAM\"");
...
void startCameraServer() {
  ...
  httpd_uri_t wifi_status_uri_slash = {
    .uri = "/wifi/status/",
    .method = HTTP_GET,
    .handler = wifi_status_handler,
    .user_ctx = NULL
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &wifi_status_uri_slash);
  }
